...
if [[ -f outputs/n08_o10_stacked_v11.running/progress.jsonl ]]; then
  progress="outputs/n08_o10_stacked_v11.running/progress.jsonl"
elif [[ -f outputs/n08_o10_stacked_v11/run/progress.jsonl ]]; then
  progress="outputs/n08_o10_stacked_v11/run/progress.jsonl"
else
  echo "No N08+O10 stacked v1.1 progress file exists yet." >&2
  exit 1
fi
...
if "learning_rate" in last:
    print("Last learning rate:", last["learning_rate"])
if "horizon" in last:
...
print("Note: ETA covers only the active stage; later stages are conditional.")
PY
