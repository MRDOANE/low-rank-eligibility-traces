...
output_root="outputs/n08_o10_stacked_v10"
running_root="${output_root}.running"
result_zip="outputs/n08_o10_stacked_v10_results.zip"
console_log="outputs/n08_o10_stacked_v10_console.log"
if [[ -e "${output_root}" || -e "${running_root}" || -e "${result_zip}" ]]; then
  echo "Refusing to overwrite an existing N08+O10 stacked v1.0 output." >&2
...
"${python_bin}" -m unittest \
  ...
  tests.test_n08_o10_stacked_experiment -v

started_at="${SECONDS}"
set +e
"${python_bin}" -m adaptive_memory.n08_o10_stacked_experiment \
  ...
  --boundary-steps 450 \
  --batch-size 8 \
  --primary-eval-examples 256 2>&1 | tee "${console_log}"
status=${PIPESTATUS[0]}
set -e
...
if [[ "${status}" -ne 0 ]]; then
  echo "N08+O10 stacked cascade failed with status ${status}." >&2
...
echo
echo "N08+O10 stacked cascade complete. Attach this one results archive:"
ls -lh "${result_zip}"
